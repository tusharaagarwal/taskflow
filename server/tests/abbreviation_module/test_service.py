...
from abbreviation_module.service import AbbreviationService, get_abbreviation_service, reset_abbreviation_service
from abbreviation_module.config import AbbreviationConfig, CacheConfig
from abbreviation_module.exceptions import AbbreviationNotFoundError, AbbreviationDatabaseError
...
class TestAbbreviationService:
    ...
    async def test_get_abbreviation_cache_hit(self, mock_db):
        ...
        # Pre-populate cache
        service._cache.set("abbr:ANNUAL", "ANN")
        
        result = await service.get_abbreviation(mock_db, "ANNUAL")
    ...
    async def test_get_abbreviation_cache_miss_db_hit(self, mock_db):
        ...
        cached = service._cache.get("abbr:ANNUAL")
        assert cached == "ANN"
    
    @pytest.mark.asyncio
    async def test_get_abbreviation_normalizes_input(self, mock_db):
        """Test that document type is normalized (uppercase, trimmed)."""
        service = AbbreviationService()
        
        # Pre-populate cache with uppercase key
        service._cache.set("abbr:ANNUAL", "ANN")
    ...
    async def test_get_abbreviation_not_found_raises(self, mock_db):
        ...
        with pytest.raises(AbbreviationNotFoundError) as exc_info:
    ...
    async def test_get_abbreviation_not_found_returns_default(self, mock_db, config_no_raise):
        ...
        service = AbbreviationService(config=config_no_raise)
    ...
    async def test_get_abbreviation_uses_fallback(self, mock_db, config_with_fallback):
        ...
        service = AbbreviationService(config=config_with_fallback)
    ...
    def test_invalidate_cache_all(self):
        ...
        assert count == 2
        assert service._cache.get("abbr:ANNUAL") is None
    ...
    def test_invalidate_cache_specific(self):
        ...
        count = service.invalidate_cache(document_type="ANNUAL")
    ...
    async def test_get_all_abbreviations(self, mock_db):
        ...
        mock_abbr1.to_dict.return_value = {"document_type": "ANNUAL", "abbreviation": "ANN"}
        ...
        assert len(result) == 2
        assert result[0]["document_type"] == "ANNUAL"
